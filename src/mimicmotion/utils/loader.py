...
def create_pipeline(base_model_path, ckpt_path, device):
    ...
    """
    mimicmotion_models = MimicMotionModel(base_model_path)
    mimicmotion_models.load_state_dict(torch.load(ckpt_path, map_location="mps", weights_only=True), strict=False)
