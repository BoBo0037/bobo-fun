...
from src.StoryDiffusion.gradio_utils import (
    character_to_dict,
...
if is_torch2_available():
    from src.StoryDiffusion.gradio_utils import AttnProcessor2_0 as AttnProcessor
else:
    from src.StoryDiffusion.gradio_utils import AttnProcessor
from huggingface_hub import hf_hub_download
from diffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl import (StableDiffusionXLPipeline)
from diffusers.schedulers.scheduling_ddim import DDIMScheduler
import torch.nn.functional as F
from diffusers.utils.loading_utils import load_image
from src.StoryDiffusion.utils import get_comic
from src.StoryDiffusion.style_template import styles
from src.StoryDiffusion.load_models_utils import get_models_dict, load_models

#################################################
DEFAULT_STYLE_NAME = "Japanese Anime"
OUTPUT_PATH = "./output_story_diffusion"
FONT_PATH = "./assets/fonts"
PHOTOMAKER_PATH = "~/.cache/huggingface/hub/models--photomaker-v1/"
...
local_dir = os.path.expanduser(PHOTOMAKER_PATH)
...
def save_results(unet, img_list):
    ...
    folder_name = f"{OUTPUT_PATH}/{timestamp}"
...
def process_generation(
    ...
    if _comic_type != "No typesetting (default)":
        ...
        font_path = os.path.join(FONT_PATH, font_choice)
...
if single_files:
    pipe = StableDiffusionXLPipeline.from_single_file(sd_model_path, torch_dtype=torch.bfloat16)
else:
    pipe = StableDiffusionXLPipeline.from_pretrained(sd_model_path, torch_dtype=torch.bfloat16, use_safetensors=False)
pipe = pipe.to(device)
pipe.enable_freeu(s1=0.6, s2=0.4, b1=1.1, b2=1.2)
# pipe.scheduler = DDIMScheduler.from_config(pipe.scheduler.config)
            ...
            with gr.Column(visible=True) as gen_prompt_vis:
                ...
                with gr.Accordion("(4) Tune the hyperparameters", open=True):
                    font_choice = gr.Dropdown(
                        label="Select Font",
                        choices=[
                            f for f in os.listdir(FONT_PATH) if f.endswith(".ttf")
                        ],
                        value="Inkfree.ttf",
    ...
    gr.Examples(
        examples=[
            [
                ...
                get_image_path_list("./assets/imgs/StoryDiffusion/taylor"),
                512,
                512,
            ],
            [
                ...
                "Comic book",
                "Using Ref Images",
                get_image_path_list("./assets/imgs/StoryDiffusion/twoperson"),
                1024,
                1024,
            ],
            [
